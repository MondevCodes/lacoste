...
export class EvaluationFormInteractionHandler extends InteractionHandler {
  ...
  public override async run(interaction: ButtonInteraction) {
    ...
    if (!targetHabbo) {
      await i.editReply({
        content:
          "Não foi possível encontrar o usuário avaliado no Habbo, verifique se a conta do mesmo no jogo está como pública.",
      });
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields([
        ...
        },
        {
          ...
          value: targetDB?.latestPromotionDate
            ? new Date(targetDB?.latestPromotionDate).toLocaleDateString(
                "pt-BR"
              )
            : "N/D",
        },
      ...
      .setAuthor({
        name: authorDB.habboName ?? authorHabbo.name,
