...
export class FollowUpFormInteractionHandler extends InteractionHandler {
    ...
    const finalRate = Number.parseInt(result.QuestionOne) + Number.parseInt(result.QuestionTwo) + Number.parseInt(result.QuestionThree) + Number.parseInt(result.QuestionFour) + Number.parseInt(result.QuestionFive) + Number.parseInt(result.QuestionSix) + Number.parseInt(result.QuestionSeven);
			...
			.addFields([
        {
          ...
          value: `${habboInteractionName ?? `@${interaction.user.tag}`}`,
				...
				},
				{
					...
					value: Number.parseInt(result.QuestionSeven) < 2 && Number.parseInt(result.QuestionSeven) >= 0  ? `${result.QuestionSeven}/1` : "N/A",
					inline: true,
				},
				{
