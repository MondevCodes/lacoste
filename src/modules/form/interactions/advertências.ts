...
export class WarningInteractionHandler extends InteractionHandler {
	public override async parse(interaction: ButtonInteraction) {
		...
		return interaction.customId === FormIds.Advertir
			? this.some()
			: this.none();
	}
	...
	public override async run(interaction: ButtonInteraction) {
		...
		const channel = await guild.channels.fetch(
			ENVIRONMENT.NOTIFICATION_CHANNELS.FORM_WARNINGS,
		);
